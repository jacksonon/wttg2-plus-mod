...
namespace ASoft.WTTG2
{
	public class OptionsUtil : MonoBehaviour
	{
		public static void BuildOptionsButton(string Text, string PlayerPrefsID, int PlayerPrefsDefault, float VerticalPos, Action ApplyON = null, Action ApplyOFF = null)
		{
			...
			GameObject gameObject = Object.Instantiate<GameObject>(GameObject.Find("NuidtyTitle"));
			bool onOff = false;
			gameObject.transform.position = new Vector2(transform.position.x - 100f, transform.position.y - VerticalPos);
			...
			GameObject ButtonOn = Object.Instantiate<GameObject>(gameObject2);
			ButtonOn.transform.position = new Vector2(gameObject2.transform.position.x, gameObject2.transform.position.y - (VerticalPos + 2f));
			ButtonOn.transform.SetParent(transform2);
			GameObject gameObject3 = GameObject.Find("NudityOffButton");
			GameObject ButtonOff = Object.Instantiate<GameObject>(gameObject3);
			ButtonOff.transform.position = new Vector2(gameObject3.transform.position.x, gameObject3.transform.position.y - (VerticalPos + 2f));
